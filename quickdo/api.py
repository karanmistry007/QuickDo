...
def get_quickdo_with_categories(
	...
	try:
		...
		for category in categories_list:
			...
			hash_map[category.parent]["categories"].append({"category": category.category})

		# ? MAP THE CHILD RECORDS WITH PARENTS
		for quickdo in quickdo_list:
			quickdo_data = quickdo
			quickdo_data["categories"] = hash_map.get(quickdo.name, {}).get("categories", [])
...
def get_upcoming_important_quickdo():
	try:
		...
		frappe.log_error("Error While Getting Total Upcoming Important QuickDos", str(e))
		return {
			"success": False,
	...
	try:
		...
		hash_map = {
			category.get("category"): sum(
				1 for quickdo in quickdo_data_list if quickdo.get("category") == category.get("category")
			)
			for category in quickdo_category_list
		}

		# ? GET CATEGORY WISE QUICKDO COUNT
		quickdo_count = [{"category": key, "quickdo": value} for key, value in hash_map.items()]
