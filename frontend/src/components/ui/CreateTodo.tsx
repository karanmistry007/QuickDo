            ...
            <div className="create-todo-container p-4 sm:p-5 w-full">
                <div className="create-todo bg-white py-2 px-4 sm:p-5 flex flex-col shadow-[0px_0px_15px_0px_rgba(0,0,0,0.1)] rounded-md">
                    ...
                    <div className="row-1 flex gap-1 sm:gap-5 justify-between items-center">
                        {/* SAVE TICK BOX */}
                        <div className={`save hover:bg-gray-100-todo-button bg-transparent rounded-full p-0.5 w-fit text-xs sm:text-sm border border-gray-600 cursor-pointer`} onClick={() => { handleSaveTodo() }} title="Create" >
                            <FaCheck className={`${saveNewTodo ? "opacity-1" : "opacity-0"}`} />
                        </div>
                        {/* END SAVE TICK BOX */}

                        {/* TODO INPUT BOX */}
                        <input type="text" className="outline-0 placeholder:text-gray-700 py-1.5 px-2 w-full" placeholder="Add a Task..." name="createTodo" id="createTodo" value={descriptionTodo} onChange={(e) => { setDescriptionTodo(e.target.value) }} onKeyDown={(e) => { e.key === "Enter" ? handleSaveTodo() : "" }} />
                        {/* END TODO INPUT BOX */}
                    ...
                    <div className={`row-2 flex gap-2.5 sm:gap-5 justify-start items-center sm:mt-2.5 pt-2 sm:pt-2.5 border-t ${isMobileScreen ? (showMoreOptions ? "show" : "hidden") : "show"}`}>
                        ...
                        <DropdownMultiSelect position={"right"} showCategories={showCategories} allCategories={allCategories} selectedCategories={selectedCategories} handleSelectedCategories={handleSelectedCategories} />
