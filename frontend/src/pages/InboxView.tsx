        ...
        const fetchAPI = async () => {
            try {
                ...
                if (response.data.message) {
                    ...
                    const groupedInventory = finalData.reduce<useAllQuickDoData[]>((acc: any, item: useAllQuickDoData) => {
                        const status: Status = item.status || "Open"; // Assuming item.status is always defined

                        // Check if the status already exists in the accumulator
                        if (!acc[status]) {
                            acc[status] = []; // Create a new array if it doesn't exist
                        }
                        acc[status].push(item); // Push the current item to the corresponding status
                        return acc; // Return the accumulator for the next iteration
                    }, []);

                    setOpenQuickDoData(groupedInventory);
